...
def _project_current_blocked_ai_read_item(
    item: Mapping[str, object],
    *,
    sheet_id: int,
    business_date: str,
    ...
    decision_lineage = (
        ...
        item.get("scheduling_gate_state"),
        item.get("scheduling_gate_reason"),
    )
    canonical_decision_lineages = {
        ("ready", "no_requirement", "allowed", "no_requirement"),
        ("ready", "requirement_confirmed", "allowed", "requirement_confirmed"),
        ("not_ready", "requirement_pending", "warning", "requirement_pending"),
    }
    ...
    return {
        ...
        "planned_at": item["planned_at"],
        "work_content": item["work_content"],
        "formal_approval_state": formal_approval_state,
        "scheduling_gate_state": item["scheduling_gate_state"],
        "scheduling_gate_reason": item["scheduling_gate_reason"],
    }


def _build_current_blocked_ai_read_result(
    payload: Mapping[str, object],
    *,
    intent: Mapping[str, object],
    scope: Mapping[str, int],
    ...
    if not isinstance(payload, Mapping):
        raise AIReadOrchestrationError("ai_read_source_shape_mismatch")
    summary = payload.get("summary")
    full_items = payload.get("blocked_entries")
    if not isinstance(summary, Mapping) or not isinstance(full_items, list):
        raise AIReadOrchestrationError("ai_read_source_shape_mismatch")
    summary_count = summary.get("blocked_count")
    if isinstance(summary_count, bool) or not isinstance(summary_count, int) or summary_count < 0:
        raise AIReadOrchestrationError("ai_read_source_shape_mismatch")
    if summary_count != len(full_items):
        raise AIReadOrchestrationError("ai_read_source_shape_mismatch")
    ...
    for item in full_items:
        if not isinstance(item, Mapping):
            raise AIReadOrchestrationError("ai_read_source_shape_mismatch")
        projected_item = _project_current_blocked_ai_read_item(
            item,
            sheet_id=int(scope["sheet_id"]),
            business_date=business_date,
        )
    ...
    evidence = {
        ...
        "as_of": as_of.isoformat(),
        "item_count": total_count,
        "empty_state_reason": "no_blocked_items_for_business_date" if total_count == 0 else None,
    }
    required_evidence_fields = intent.get("evidence_fields")
    if not isinstance(required_evidence_fields, (list, tuple)) or not set(required_evidence_fields).issubset(
        evidence
...
def build_role_scoped_ai_read_result(
    ...
    *,
    ...
    if intent_key == "current_blocked_items":
        ...
        return _build_current_blocked_ai_read_result(
            payload,
            intent=intent,
            scope=scope,
            actor_role=actor_role,
