    ...
    try:
        with app.db() as conn:
            row = conn.execute(
                ...
                ORDER BY f.sort_order, u.sort_order, u.id
                LIMIT 1
                """
            ).fetchone()
            if row is None:
                print("FAIL no units rows found")
                return 1

            result, error, details = write_service._write_unit_name_to_postgres_secondary(  # type: ignore[attr-defined]
                conn,
                unit_id=row["id"],
                name=row["name"],
            )
