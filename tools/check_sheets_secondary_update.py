...
def main() -> int:
    ...
    try:
        with app.db() as conn:
            sheet = conn.execute("SELECT id, name FROM sheets ORDER BY sort_order, id LIMIT 1").fetchone()
            if sheet is None:
                print("FAIL no sheet rows found")
                return 1

            result, error, details = write_service._write_sheet_name_to_postgres_secondary(  # type: ignore[attr-defined]
                conn,
                sheet_id=sheet["id"],
                name=sheet["name"],
            )
