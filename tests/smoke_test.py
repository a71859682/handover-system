...
def test_controlled_dual_write_floors_update_reuses_postgres_compat_primary_connection():
    os.environ["DUAL_WRITE_ENABLED"] = "true"
    os.environ["DUAL_WRITE_TABLES"] = "meta,sheets,extra_fields,units,floors"
    os.environ["DUAL_WRITE_STRICT"] = "false"
    os.environ["DUAL_WRITE_DRY_RUN"] = "true"
    os.environ["DATABASE_URL"] = "postgresql://example.test/app"
    ...
    log_output = "\n".join(messages)
    assert any("SAVEPOINT dual_write_floors_secondary" in call[0] for call in raw_calls if isinstance(call[0], str))
    assert any("UPDATE floors" in call[0] for call in raw_calls if isinstance(call[0], str))
    assert "DUAL_WRITE operation=update table=floors" in log_output
    assert "postgres_result=success" in log_output
    assert "DUAL_WRITE_DRY_RUN operation=update table=floors" in log_output
    assert "DUAL_WRITE_FLOORS_SECONDARY table=floors strategy=reuse_primary_postgres_connection event=SAVEPOINT_START" in log_output
    assert "DUAL_WRITE_FLOORS_SECONDARY table=floors strategy=reuse_primary_postgres_connection event=EXECUTE_SQL_OK" in log_output


def test_controlled_dual_write_floors_update_strict_false_logs_postgres_error_without_blocking_primary():
    os.environ["DUAL_WRITE_ENABLED"] = "true"
    os.environ["DUAL_WRITE_TABLES"] = "meta,sheets,extra_fields,units,floors"
    os.environ["DUAL_WRITE_STRICT"] = "false"
    os.environ["DUAL_WRITE_DRY_RUN"] = "true"
    ...
    class FailingCursor:
        def execute(self, sql, params=None):
            raise RuntimeError("forced floors secondary failure")
    ...
    conn.execute("INSERT INTO floors (id, sheet_id, sort_order, name, block_name, unit_count) VALUES (1, 1, 1, 'Before', 'A', 8)")
    write_service.psycopg = FailingPsycopg()
    ...
    log_output = "\n".join(messages)
    stored_row = conn.execute("SELECT name, block_name, unit_count FROM floors WHERE id = 1").fetchone()
    assert stored_row["name"] == "After"
    assert stored_row["block_name"] == "B"
    assert stored_row["unit_count"] == 8
    assert "DUAL_WRITE operation=update table=floors" in log_output
    assert "postgres_result=failed" in log_output
    assert "DUAL_WRITE_DRY_RUN operation=update table=floors" in log_output
    assert failing_connections and failing_connections[0].rolled_back is True
...
def test_controlled_dual_write_tasks_update_reuses_postgres_compat_primary_connection():
    os.environ["DUAL_WRITE_ENABLED"] = "true"
    os.environ["DUAL_WRITE_TABLES"] = "meta,sheets,extra_fields,units,floors,tasks"
    os.environ["DUAL_WRITE_STRICT"] = "false"
    os.environ["DUAL_WRITE_DRY_RUN"] = "true"
    os.environ["DATABASE_URL"] = "postgresql://example.test/app"
    ...
    try:
        write_service.update_task_fields_sqlite(
            compat_conn,
            31,
            vendor="Vendor A",
            location="Location A",
    ...
    log_output = "\n".join(messages)
    assert any("SAVEPOINT dual_write_tasks_secondary" in call[0] for call in raw_calls if isinstance(call[0], str))
    assert any("UPDATE tasks" in call[0] for call in raw_calls if isinstance(call[0], str))
    assert "DUAL_WRITE operation=update table=tasks" in log_output
    assert "postgres_result=success" in log_output
    assert "DUAL_WRITE_DRY_RUN operation=update table=tasks" in log_output
    assert "DUAL_WRITE_TASKS_SECONDARY table=tasks strategy=reuse_primary_postgres_connection event=SAVEPOINT_START" in log_output
    assert "DUAL_WRITE_TASKS_SECONDARY table=tasks strategy=reuse_primary_postgres_connection event=EXECUTE_SQL_OK" in log_output


def test_controlled_dual_write_tasks_update_strict_false_logs_postgres_error_without_blocking_primary():
    os.environ["DUAL_WRITE_ENABLED"] = "true"
    os.environ["DUAL_WRITE_TABLES"] = "meta,sheets,extra_fields,units,floors,tasks"
    os.environ["DUAL_WRITE_STRICT"] = "false"
    os.environ["DUAL_WRITE_DRY_RUN"] = "true"
    ...
    class FailingCursor:
        def execute(self, sql, params=None):
            raise RuntimeError("forced tasks secondary failure")
    ...
    conn.execute(
        "INSERT INTO tasks (id, sheet_id, col_index, vendor, location, name) VALUES (1, 1, 4, 'Vendor before', 'Location before', 'Task before')"
    )
    write_service.psycopg = FailingPsycopg()
    ...
    log_output = "\n".join(messages)
    stored_row = conn.execute("SELECT vendor, location, name FROM tasks WHERE id = 1").fetchone()
    assert stored_row["vendor"] == "Vendor after"
    assert stored_row["location"] == "Location after"
    assert stored_row["name"] == "Task after"
    assert "DUAL_WRITE operation=update table=tasks" in log_output
    assert "postgres_result=failed" in log_output
    assert "DUAL_WRITE_DRY_RUN operation=update table=tasks" in log_output
    assert failing_connections and failing_connections[0].rolled_back is True
...
def run():
    ...
    test_controlled_dual_write_floors_update_reuses_postgres_compat_primary_connection()
    test_controlled_dual_write_tasks_update_reuses_postgres_compat_primary_connection()
    ...
    test_controlled_dual_write_floors_update_strict_false_logs_postgres_error_without_blocking_primary()
    test_controlled_dual_write_tasks_update_strict_false_logs_postgres_error_without_blocking_primary()
